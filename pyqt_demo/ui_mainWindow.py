...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.plotWidget_test3 = PlotWidget(self.groupBox_2)
        self.plotWidget_test3.setObjectName("plotWidget_test3")
        self.verticalLayout_5.addWidget(self.plotWidget_test3)
        self.plotWidget_test4 = PlotWidget(self.groupBox_2)
        self.plotWidget_test4.setObjectName("plotWidget_test4")
        self.verticalLayout_5.addWidget(self.plotWidget_test4)
    ...
    def retranslateUi(self, MainWindow):
        ...
        MainWindow.setWindowTitle(_translate("MainWindow", "IAT_Project"))
    ...
    def get_standing_query():
        zundfehler = '/Users/bytedance/iat-project/input/standing_query/zundfehler.txt'
        values = []
        with open(zundfehler, 'r') as f:
            for idx, line in enumerate(f.readlines()):
                line1 = line.strip().split()
                values.append(float(line1[1]))
        return values
    ...
    def run_signal(self):
        ...
        self.consumer_voltage = KafkaConsumer("Voltage", bootstrap_servers=["localhost:9092"],
                                              auto_offset_reset='latest',
                                              consumer_timeout_ms=6000)
        self.length = 2000  # 横坐标长度
        self.judgement_length = 2000  # 横坐标长度
        self.ptr = 0
        self.t = []
        ...
        self.query = self.plotWidget_test3.plot(np.zeros(self.judgement_length).__array__('d'), name="mode2")
        self.standing_query = self.plotWidget_test4.plot(np.zeros(self.judgement_length).__array__('d'), name="mode2")
        # self.standing_query.setData([0, 1, 2, 3, 4, 3, 2, 1])  # LSTM
        self.standing_query.setData(self.get_standing_query())
        ...
        # 加载模型
        self.model = load_model('/Users/bytedance/iat-project/LSTM/output/current_without_zundfehler.h5')

        th1 = threading.Thread(target=self.get_data_current)
        th1.start()
        th2 = threading.Thread(target=self.get_data_voltage)
        th2.start()
        # th3 = threading.Thread(target=self.predict)
        # th3.start()

    def watch(self):
        while True:
            if self.flag:
                self.standing_query.setData(self.data_current)
                self.standing_query.setPos(self.ptr, 0)
                self.flag = False
    ...
    def predict_simple(self, t, v):
        # print('predict_simple')
        t = t - v
        # self.query.setData(self.data_current)
        # self.query.setPos(self.ptr, 0)  # 重新设定 x 相关的坐标原点
        if abs(t.sum()) <= 20 and v > 250:
            print('plot')
            print(abs(t.sum()), v)
            self.query.setData(self.data_current)
            self.query.setPos(self.ptr, 0)  # 重新设定 x 相关的坐标原点
            self.flag = True
            self.lineEdit.setText(str(self.ptr+1800))
            self.lineEdit_2.setText(str(v))
            self.lineEdit_3.setText(str(self.t[-1]))

    def predict(self):
        while True:

            # if len(self.data_current) >= self.judgement_length:
            # if self.ptr % 4 == 0:
            ptr, tmp = self.ptr, self.data_current[-self.judgement_length:]
            t = np.array([tmp])
            t = np.reshape(t, (t.shape[0], t.shape[1], 1))  # 维度转换
            res = self.model.predict(t)
            # print('predict:', res)
            if res > 0.9:
                print('predict', len(self.data_current) >= self.judgement_length, res, tmp)
                self.query.setData(tmp)
                self.query.setPos(ptr, 0)  # 重新设定 x 相关的坐标原点
    ...
    def get_data_current(self):
        ...
        for msg in self.consumer_current:
            ...
            if self.data_current[-2] == self.value_current[-1] and self.value_current[-1] != 0:
                t = np.array(copy.deepcopy(self.data_current[-200:]))
                v = self.value_current[-1]
                self.predict_simple(t, v)
    ...
    def find_extreme_value(self, data, flag):
        if self.ptr % self.judgement_length == 0:
            data = data[-self.judgement_length:]
